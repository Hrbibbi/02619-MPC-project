...
for idx in range(2):
    ...
    determ.step_response(idx, p.h0, p.ubar, p.d_determ, tf, normalized=True, plot_title=plot_title)
...
for i, (f1, f2, name) in enumerate(noise_levels):
    ...
    for idx in range(2):
        ...
        stoch_piece.step_response(idx, p.h0, p.ubar, p.mu_d, tf, plot_title=plot_title, measurements=True)