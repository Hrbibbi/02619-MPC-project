...
stoch_brown.u = u_set
MPC = FourTank_MPC(stoch_brown, m_ss, determ.u, d_ss,R,I_C,O_C,static_kf=False,MPC_type="linear",use_kf=True,NL_sim=True)
T = 5
h0 = np.concatenate((p.h0, Fbar))
#h0 = np.concatenate((np.array([50,50,50,50]),Fbar))
MPC.simulate(p.t0,p.tf,h0,T)
plot_hist(MPC, f"Closed-loop LMPC-controller-inputoutputconstraint - SDE", filename=f'LMPC_closetoSS',step=False,NL=True,state_est=False)